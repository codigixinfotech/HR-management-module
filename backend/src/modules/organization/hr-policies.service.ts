...
@Injectable()
export class HrPoliciesService implements OnModuleInit {
  ...
  async getKpis(companyId?: string) {
    ...
    const overallRate =
      totalEmployees > 0 ? Math.round((totalSigned / totalEmployees) * 100) : 100;
  ...
  async findOne(id: string) {
    ...
    // Fetch version history for the same policy code
  ...
  async create(dto: CreateHrPolicyDto) {
    ...
    if (existing) {
      throw new ConflictException(`Policy code ${dto.policyCode} is already active.`);
    }
  ...
  async update(id: string, dto: UpdateHrPolicyDto) {
    ...
    if (dto.policyCode && dto.policyCode !== existing.policyCode) {
      ...
      if (dup) throw new ConflictException(`Policy code ${dto.policyCode} already exists.`);
    }
  ...
  async createVersion(id: string, dto: CreatePolicyVersionDto) {
    const parent = await this.prisma.hrPolicy.findUnique({ where: { id } });
    if (!parent) throw new NotFoundException(`Policy ${id} not found`);
    ...
    // Create new version as current active
    return this.prisma.hrPolicy.create({
      data: {
        ...
        signedCount: 0, // reset sign-off count for new version
