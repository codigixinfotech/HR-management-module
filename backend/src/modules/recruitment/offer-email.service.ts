...
Codigix Infotech Pvt. Ltd.`;
  ...
  async sendInterviewDetailsEmail(dto: {
    candidateName: string;
    candidateEmail: string;
    position: string;
    ...
    const subject = `Interview Invitation – ${dto.position}`;
    const teamsBtnHtml = dto.teamsMeetingUrl
      ? `
        <div style="text-align: center; margin: 24px 0;">
          <a href="${dto.teamsMeetingUrl}" target="_blank" style="background: #4f46e5; color: white; padding: 12px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 14px; display: inline-block;">
            Join Microsoft Teams Meeting →
          </a>
        </div>
        <p style="font-size: 12px; color: #64748b; text-align: center;">
          Direct Teams Link: <a href="${dto.teamsMeetingUrl}" style="color: #4f46e5;">${dto.teamsMeetingUrl}</a>
        </p>
      `
      : '';
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
        ...
        <div style="padding: 24px; color: #1e293b;">
          <p style="font-size: 15px; font-weight: bold; margin-top: 0;">Hello ${dto.candidateName},</p>
          <p style="font-size: 13px; line-height: 1.6; color: #475569;">
            Your interview has been scheduled for the <strong>${dto.position}</strong> position.
          </p>

          <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; margin: 16px 0;">
            <table style="width: 100%; font-size: 13px; border-collapse: collapse;">
              <tr><td style="padding: 4px 0; color: #64748b; width: 140px;">Position:</td><td style="font-weight: bold; color: #4f46e5;">${dto.position}</td></tr>
              <tr><td style="padding: 4px 0; color: #64748b;">Interview Date:</td><td style="font-weight: 600;">${dto.interviewDate}</td></tr>
              <tr><td style="padding: 4px 0; color: #64748b;">Start Time:</td><td style="font-weight: 600;">${dto.startTime}</td></tr>
              <tr><td style="padding: 4px 0; color: #64748b;">Duration:</td><td style="font-weight: 600;">${dto.durationMinutes} Minutes</td></tr>
              <tr><td style="padding: 4px 0; color: #64748b;">Interview Format:</td><td style="font-weight: 600; color: #4f46e5;">${dto.interviewFormat}</td></tr>
              <tr><td style="padding: 4px 0; color: #64748b;">Interviewer Panel:</td><td style="font-weight: 600;">${dto.interviewerName}</td></tr>
            </table>
          </div>
    ...
    try {
      ...
      const info = await transporter.sendMail({
        ...
        to: dto.candidateEmail,
        subject,
        text: `Hello ${dto.candidateName}, Your interview for ${dto.position} is scheduled on ${dto.interviewDate} at ${dto.startTime}. Format: ${dto.interviewFormat}.`,
        html: htmlContent,
      });
      this.logger.log(`Interview details email sent to ${dto.candidateEmail} via SMTP! MessageId: ${info.messageId}`);
      return { success: true, message: `Interview details email sent successfully to ${dto.candidateEmail}`, messageId: info.messageId };
    } catch (err: any) {
      this.logger.error(`Failed to send interview details email to ${dto.candidateEmail}: ${err.message}`);
      return { success: false, message: `Gmail SMTP Error: ${err.message}`, error: err.message };
    }
  }
}
