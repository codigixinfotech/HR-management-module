...
export class JobOpeningsService {
  ...
  // ── Portal Configuration & Paginated Public API ──
  private portalConfigStore: Record<string, any> = {
  ...
  updatePortalConfig(dto: Record<string, any>) {
  ...
  async listPublicJobsPaginated(params: {
    ...
    let filtered = sourceJobs.filter((job) => {
      ...
      const deptName = job.department?.name || (job as any).departmentName || '';
      const compName = (job as any).company?.name || '';
      ...
      const matchesDept =
        deptFilter === 'ALL' || deptName.toLowerCase() === deptFilter.toLowerCase();
