...
let TravelExpenseService = class TravelExpenseService {
    ...
    async getBooking(id) {
        ...
        if (!booking) {
            throw new common_1.NotFoundException(`Travel Booking with ID ${id} not found`);
        }
        return booking;
    }
    async createBooking(dto, actorName = 'Employee') {
        ...
        const targetCompanyId = dto.companyId && dto.companyId !== 'ALL' && dto.companyId.trim() !== ''
            ? dto.companyId
            : employee.companyId;
    ...
    async updateBooking(id, dto, actorName = 'Employee') {
        const existing = await this.getBooking(id);
        if (existing.status !== 'DRAFT' && existing.status !== 'REJECTED') {
            throw new common_1.BadRequestException(`Cannot edit travel request in status ${existing.status}`);
        }
    ...
    async createExpenseClaimFromBooking(bookingId, dto) {
    ...
    async getDashboardStats(companyId) {
        ...
        const pendingClaimsAmount = pendingClaims.reduce((acc, c) => acc + Number(c.amount || 0), 0) +
    ...
    async updateClaimStatus(id, dto) {
        ...
        if (!claim) {
            throw new common_1.NotFoundException(`Expense Claim ${id} not found`);
        }
