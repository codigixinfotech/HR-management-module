...
let ExitsService = class ExitsService {
    ...
    async getKpis(companyId) {
        ...
        const activeExits = allExits.filter((e) => !['EXITED', 'OFFBOARDING_COMPLETED', 'REJECTED', 'WITHDRAWN'].includes(e.status));
        const pendingApprovals = allExits.filter((e) => ['INITIATED', 'HR_REVIEW', 'MANAGER_APPROVAL', 'FINAL_APPROVAL'].includes(e.status));
        ...
        const exitsThisMonth = allExits.filter((e) => ['EXITED', 'OFFBOARDING_COMPLETED'].includes(e.status) &&
            e.updatedAt >= startOfMonth).length;
        let totalExitDays = 0;
        allExits.forEach((e) => {
    ...
    async findAll(search, status, companyId) {
        const where = {};
        if (companyId)
    ...
    async findOne(id) {
        ...
        if (!exit)
            throw new common_1.NotFoundException('Exit offboarding record not found');
        return exit;
    }
    async create(dto) {
    ...
    async updateClearanceItem(itemId, dto) {
        ...
        if (allCleared)
            overallClearanceStatus = 'COMPLETED';
        else if (anyInFilter)
    ...
    async saveExitInterview(exitId, dto) {
    ...
    async saveFnfSettlement(exitId, dto) {
        const exit = await this.findOne(exitId);
    ...
    async completeExit(id, performedBy) {
        const exit = await this.findOne(id);
        if (exit.clearanceStatus !== 'COMPLETED') {
            throw new common_1.BadRequestException('Cannot grant Final Exit Approval: Department clearances are not 100% completed.');
        }
        if (exit.exitInterviewStatus !== 'COMPLETED') {
            throw new common_1.BadRequestException('Cannot grant Final Exit Approval: Exit Interview questionnaire has not been completed.');
        }
        if (exit.fnfStatus !== 'COMPLETED') {
            throw new common_1.BadRequestException('Cannot grant Final Exit Approval: Full & Final Settlement (F&F) is pending finance approval.');
        }
